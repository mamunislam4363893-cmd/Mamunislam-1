...
function renderRecentActivity(history) {
    ...
    container.innerHTML = history.map(item => {
        ...
        if (item.reward) {
            ...
            rewardDisplay = (isPos ? '+' : (isNeg ? '-' : '')) + Math.abs(amt) + ' ' + String(asset).toUpperCase();
        }
...
function renderBalances() {
    ...
    if (elTc) elTc.innerText = tokens.toLocaleString();
    if (elJs) elJs.innerText = gems.toLocaleString();
    if (elUsd) elUsd.innerText = '$' + usd.toFixed(2);
    ...
    if (hTc) hTc.innerText = tokens.toLocaleString();
    if (hJs) hJs.innerText = gems.toLocaleString();
    if (hUsd) hUsd.innerText = '$' + usd.toFixed(2);
